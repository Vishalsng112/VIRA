...
import time
import uuid
from vira.agent_orchestration import registry
from vira.agent_runtime.runtime import AgentRuntime, AgentContext
from vira.agent_orchestration.registry import AgentCapability, AgentState
...
class BaseAgent(ABC):
    ...
    async def can_handle(self, event: Event) -> bool:
        # Default: check if event.type is in subscribed_events
        result = event.type in self._subscribed_events
        logger.debug(f"[AGENT {self.name}] can_handle({event.type}) -> {result}")
        return result
    ...
    async def send_event(self, event_type: str, data: Any, target_agent_ids: Optional[List[str]] = None):
        ...
        if self.runtime:
            ...
            raise RuntimeError("Runtime not set")